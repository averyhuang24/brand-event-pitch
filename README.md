...
- 支持可选趋势增强包：由 agent 为小白用户配置小红书/抖音 MCP，默认只读搜索和分析，不做发布/点赞/评论
...
- 小红书：搜索笔记、推荐列表、笔记详情、互动数据、评论、用户主页。
- 抖音：搜索视频、视频详情、评论、达人/作者信息、标题/文案/语音转写或链接解析。
- 默认不做：发布、评论、点赞、收藏、关注、私信、账号运营、批量增长。

推荐优先评估：

- 小红书：`xpzouying/xiaohongshu-mcp`
- 抖音：搜索/详情/评论类 MCP；如果搜索不可用，则使用链接解析类 MCP 作为兜底
